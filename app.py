...
def get_l2_price(ex_obj, symbol, side, amount_usdt):
    """Средняя цена исполнения для объема в USDT"""
    try:
    ...
    for sym in all_syms:
        ...
        if len(ex_with_sym) >= 2:
            ...
            if p_sell > p_buy and vol >= min_vol:
                ...
                if 0.1 < diff < 15: # Отсекаем шум и ошибки API
                    pre_candidates.append({'sym': sym, 'buy_ex': buy_ex, 'sell_ex': sell_ex, 'diff': diff})

    # 3. Анализ L2 стаканов для ТОП-10
...
with st.sidebar:
    ...
    fee = st.number_input("Комиссия Taker %", 0.0, 0.2, 0.04, step=0.01, format="%.3f")
    min_v = st.number_input("Мин. объем монеты (USDT)", 0, 1000000, 100000)
    
    # Исправленный список без SyntaxError
    refresh_opts = [15, 30, 60, 120]
    refresh = st.select_slider("Обновление (сек)", options=refresh_options, value=30)
    
    st.header("Уведомления")
    alert_p = st.slider("Звук при профите > %", 0.1, 5.0, 0.5)
...
if not df.empty:
    ...
    st.info("Поиск выгодных связок... Уменьшите 'Мин. объем' если ничего не находит.")

st.caption(f"Обновлено: {pd.Timestamp.now().strftime('%H:%M:%S')}. Текущий год: 2026")
